...
def turn_left():
    """
    Turns the robot 90 degrees to the left relative to the robot's
    center. In this context, the center is the robot's logic unit,
    which should be in the middle of two squares that each have
    one sensor over top of them.

    Note: This method is fairly consistent, but the robot's
    motors sometimes do throw off the correct positioning of
    where the robot should actually end up, so minor
    adjustments may need to be made if this causes problems.
...
def rotate_right():
	"""
	This is used to get the robot to turn back to the top of the
    board so that the orientation will stay in the same direction
